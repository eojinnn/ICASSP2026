...
from architecture.CST_details.mhsa import CorrelatedAttentionBlock
...
class CST_attention(torch.nn.Module):
    def __init__(self, temp_embed_dim, params):
        ...
        if self.ChAtten_dca:
            ...
            self.cor_mhsa = CorrelatedAttentionBlock(embed_dim=self.ch_attn_embed_dim, num_heads=params['nb_heads'],
                                                     dropout=self.dropout_rate, c_topk=2, max_lag_candidates=64)
        ...
        if self.ChAtten_ule:
            ...
            self.ch_mhsa = nn.MultiheadAttention(embed_dim=self.ch_attn_embed_dim, num_heads=10 if params['t_pooling_loc']=='end' else params['nb_heads'],
                                                 dropout=self.dropout_rate, batch_first=True)
            self.cor_mhsa = CorrelatedAttentionBlock(embed_dim=params['nb_cnn2d_filt'], num_heads=params['nb_heads'],
                                                     dropout=self.dropout_rate, c_topk=2, max_lag_candidates=64)
            self.ch_layer_norm = nn.LayerNorm(self.temp_embed_dim)
            if self.linear_layer:
                self.ch_linear = nn.Linear(self.temp_embed_dim, self.temp_embed_dim)
    ...
    def forward(self,x, M, C, T, F):
        if self.ChAtten_ule: # CST-attention(ULE)
            ...
            x_unfold = rearrange(x_unfold, 'b (c u) tf -> (b tf) u c', c=C).contiguous()

            ######################attention method adjusted#######################
            xc, _ = self.cor_mhsa(x_unfold)
